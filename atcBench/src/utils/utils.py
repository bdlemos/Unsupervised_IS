...
def split_raw_data(df_data, df_splits, fold):
    ...
    test_idxs = df_splits.loc[fold].test_idxs
...
def get_data_from_biois(config, fold):
    """Load data from bio-is texts/scores files and apply IS-filtered splits.

    Activated when the data config contains ``external_splits_path``.
    The train indices in that pkl are the IS-selected subset; the test
    indices are the same as in the original (unfiltered) split.
    """
    texts_path = config.data.texts_path
    scores_path = config.data.scores_path
    external_splits_path = config.data.external_splits_path
    ...
    # Fit label encoder on all unique labels (avoids unseen-label errors)
    y_encoder = LabelEncoder().fit(sorted(set(labels)))

    # Load IS-filtered splits
    with open(external_splits_path, 'rb') as f:
        df_splits = pd.read_pickle(f)

    train_idxs = df_splits.iloc[fold]['train_idxs']
    test_idxs  = df_splits.iloc[fold]['test_idxs']

    X_train = [texts[i]  for i in train_idxs]
    y_train = [labels[i] for i in train_idxs]
    X_test  = [texts[i]  for i in test_idxs]
    y_test  = [labels[i] for i in test_idxs]

    y_train = y_encoder.transform(y_train)
    y_test  = y_encoder.transform(y_test)

    print(f"[unsupervised-is] fold={fold} | train={len(X_train)} | test={len(X_test)}")
    return X_train, y_train, X_test, y_test
...
def getdata(config, fold):
    ...
    # --- bio-is mode: load texts/scores from file + IS-filtered split pkl ---
    if 'external_splits_path' in config.data:
        X_train, y_train, X_test, y_test = get_data_from_biois(config, fold)
        return X_train, y_train, X_test, y_test
